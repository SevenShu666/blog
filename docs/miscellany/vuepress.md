...
3.将Vuepress安装为本地依赖，官网不推荐全局安装

> 注意
>
> 如果你的现有项目依赖了 webpack 3.x，我们推荐使用Yarn而不是npm来安装 VuePress。因为在这种情形下，npm会生成错误的依赖树。

4.在package.json中, 配置命令
...
> 如果热更新失效，将vuepress dev docs修改为vuepressdev docs --temp .temp

5.创建docs目录用于放置.md类型的文章

~~~
mkdir docs
~~~

6.在docs文件夹中创建.vuepress文件夹放置相关的配置文件
...
通过items数组可以将单一跳转改为下拉列表，再嵌套items可以在列表内分组
...
你想要根据当前页面标题自动生成侧边栏，可以在config.js中配置
...
如果你想要自定义布局，需要在.vuepress文件夹下创建components文件夹，在components下再创建layout.vue文件
...
globalUIComponents接受的名字就是.vue文件名
...
这里可以使用gitTalk和valine，这里使用的是valine，进入[Valine官网](https://console.leancloud.cn/apps)，需要先注册才能使用，然后创建应用，从设置->应用凭证中可以看到**AppID**和**AppKey**
...
在.vuepress/components创建Valine.vue，使用时需要在md文件最后加入`<Valine></Valine>`
...
在github上创建一个名为blog的仓库, 并将代码提交到github上，创建一个名为gh-pages 的分支

### 配置仓库名称

配置`docs/.vuepress/config.js`文件中的base
...
每个仓库都有一个Actions，点进去，New workflow，创建一个后缀为.yml的文件
